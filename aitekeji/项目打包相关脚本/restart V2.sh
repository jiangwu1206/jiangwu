...
cp -R /usr/local/dev/$1/target/$1.jar /usr/local/atms/$1/

isRun=`ps -efww|grep -w $1 |grep -v grep |grep -v restart |cut -c 9-15|wc -l`
if [ "$isRun" -gt 0 ]
then
	ps -efww|grep -w $1 |grep -v grep |grep -v restart |cut -c 9-15|xargs kill -9
fi
